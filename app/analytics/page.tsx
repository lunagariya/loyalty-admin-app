'use client';
import {ActionList,BlockStack,Button,Card,EmptyState,IndexTable,Page,Popover,SkeletonBodyText,Text} from '@shopify/polaris';
...
export default function AnalyticsPage(){
  const analytics=useAnalytics(),top=useTopCustomers(),{notify}=useUi(),[open,setOpen]=useState(false),d=analytics.data;
  const exportFile=async(type:'customers'|'transactions'|'redemptions')=>{setOpen(false);try{await downloadCsv(type);notify(`${type} export downloaded`)}catch(error){notify(message(error),true)}};
  ...
  const activator=<Button onClick={()=>setOpen(v=>!v)}>Export CSV</Button>;
  return <><TitleBar title="Analytics"/><Page title="Analytics"><BlockStack gap="500">
    ...
    <Popover active={open} activator={activator} onClose={()=>setOpen(false)}><ActionList items={(['customers','transactions','redemptions'] as const).map(type=>({content:`Export ${type}`,onAction:()=>exportFile(type)}))}/></Popover>
