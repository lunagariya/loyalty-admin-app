...
export async function api<T>(path:string,init:RequestInit={}):Promise<T>{const headers=new Headers(init.headers);if(init.body&&!headers.has('Content-Type'))headers.set('Content-Type','application/json');const response=await fetch(`${API_ORIGIN}${path}`,{...init,headers});if(!response.ok){let payload:{error?:{message?:string;code?:string}}={};try{payload=await response.json();}catch{}const needsInstall=response.status===401||(response.status===403&&payload.error?.code==='SHOP_NOT_INSTALLED');if(needsInstall){const shop=localStorage.getItem('loyalty-shop')||window.shopify?.config.shop;if(shop){localStorage.setItem('loyalty-shop',shop);window.open(`${BACKEND}/auth?shop=${encodeURIComponent(shop)}`,'_top');throw new ApiError(response.status,'Redirecting to Shopify installation…','REAUTHORIZING')}}throw new ApiError(response.status,payload.error?.message||`Request failed (${response.status})`,payload.error?.code);}const payload=await response.json();return payload.data as T;}
