'use client';
import {AppProvider,Banner,BlockStack,Button,Frame,Page,TextField,Toast} from '@shopify/polaris';
import en from '@shopify/polaris/locales/en.json';
import {NavMenu} from '@shopify/app-bridge-react';
import {SWRConfig} from 'swr';
import {createContext,useContext,useEffect,useState} from 'react';

type UiContextValue={notify:(message:string,error?:boolean)=>void};
...
export function Providers({children}:{children:React.ReactNode}){
  const[ready,setReady]=useState(false),[needsInstall,setNeedsInstall]=useState(false),[toast,setToast]=useState<{message:string;error:boolean}|null>(null);
  useEffect(()=>{
  ...
  const notify=(message:string,error=false)=>setToast({message,error}),value={notify};
  return <AppProvider i18n={en}><SWRConfig value={{revalidateOnFocus:false,shouldRetryOnError:false}}><UiContext.Provider value={value}><Frame>
  ...
  </UiContext.Provider></SWRConfig></AppProvider>;
}
